...
VOLUME /logs
EXPOSE 9500
ENTRYPOINT exec java -server $JAVA_JVM_OPTS -Duser.timezone=GMT+08 -jar /base/$APP_NAME.jar --spring.profiles.active=pro --zzq.google=$google --zzq.smart=$smart >/logs/$APP_NAME.log 2>&1
